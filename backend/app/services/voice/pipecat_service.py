...
class PipecatService:
    ...
    def __init__(self, config: Optional[PipecatServiceConfig] = None):
        ...
        self.transport: Optional[FastAPIWebsocketTransport] = None
    ...
    async def start(self, websocket: WebSocket) -> None:
        ...
        try:
            ...
            if "websocket" in str(e).lower() or "connection" in str(e).lower():
                raise PipecatConnectionError(f"Failed to establish WebSocket connection: {e}") from e
            else:
        ...
        try:
            context = LLMContext(
                messages=[
                    {
                        "role": "system",
                        "content": self.config.pipeline.system_prompt
                    }
                ]
            )
            logger.debug(f"Created LLM context with system prompt: {self.config.pipeline.system_prompt[:50]}...")
            return context
        except Exception as e:
    ...
    def _create_aggregators(self):
        ...
        if not self.vad_analyzer:
            raise PipecatPipelineError("VAD analyzer must be initialized before creating aggregators")
        if not self.context:
            raise PipecatPipelineError("Context must be initialized before creating aggregators")
    ...
    def _create_task(self) -> PipelineTask:
        ...
        if not self.pipeline:
            raise PipecatPipelineError("Pipeline must be initialized before creating task")
