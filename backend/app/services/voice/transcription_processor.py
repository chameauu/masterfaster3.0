...
from .transcription import AudioProcessingError, TranscriptionService
...
class TranscriptionProcessor(FrameProcessor):
    ...
    def __init__(self):
        ...
        try:
            ...
            raise TranscriptionError(
                f"Transcription service initialization failed: {e}"
            ) from e
        ...
        try:
            ...
            try:
                ...
                raise AudioConversionError(
                    f"Failed to convert audio to WAV: {e}"
                ) from e
    ...
    def _convert_to_wav(
        ...
        if num_channels not in (1, 2):
            raise AudioConversionError(
                f"Invalid number of channels: {num_channels} (must be 1 or 2)"
            )
