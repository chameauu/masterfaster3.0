...
from app.db import SearchSourceConnector, Document, Chunk, get_async_session
...
async def check_status():
    ...
    async for session in get_async_session():
        ...
        for doc in all_docs:
            connector_info = f"Connector {doc.connector_id}" if doc.connector_id else "No connector (uploaded)"
        ...
        total_indexed = sum(len([d for d in all_docs if d.connector_id == conn.id]) for conn in connectors)
