...
async def test_composio():
    ...
    if not ComposioService.is_enabled():
        ...
        print(f"     COMPOSIO_API_KEY = {'SET' if config.COMPOSIO_API_KEY else 'NOT SET'}")
        return
    print(f"  ✅ PASSED: Composio is enabled")
    print(f"     API Key: {config.COMPOSIO_API_KEY[:10]}...{config.COMPOSIO_API_KEY[-4:]}")
    ...
    try:
        ...
        for toolkit in toolkits:
            print(f"     - {toolkit['name']} (id: {toolkit['id']}, indexable: {toolkit['is_indexable']})")
    except Exception as e:
        print(f"  ❌ FAILED: {e}")
        return
    ...
    try:
        ...
        if not auth_configs.items:
            ...
            print("     https://app.composio.dev/ → Integrations → Google Drive → Add Auth Config")
        else:
    ...
    try:
        ...
        print("     Connected accounts are created when users click 'Connect' in the UI")
    except Exception as e:
        print(f"  ⚠️  Note: {e}")
    ...
    print("     If enabled, token retrieval will return masked tokens (too short to use)")
