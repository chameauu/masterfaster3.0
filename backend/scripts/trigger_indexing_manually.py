...
async def trigger_indexing():
    ...
    async for session in get_async_session():
        ...
        task = index_google_drive_files_task.delay(
            connector.id, connector.search_space_id, str(connector.user_id), items_dict
        )

        print("\n✅ Task created!")
        print(f"   Task ID: {task.id}")
        print("   Task name: index_google_drive_files")
        print("\nCheck Celery worker terminal for progress.")
        print("Run 'uv run python check_indexing_status.py' to check results.")
